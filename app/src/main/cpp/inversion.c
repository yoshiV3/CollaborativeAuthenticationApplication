...
void inverse_n(uint32_t const * const a, uint32_t * const res)
{
    ...
    while((uone + vone) == 0)
    {
        ...
        while ((fbu&1) == 0)
        {
            ...
            if ((fbx&1) == 1)
            {
                ...
                uint32_t xone_add_n[SIZE + 3];
                signed_add(xone, N, xone_add_n, SIZE +1);
                for (uint8_t j =0; j < SIZE+1; j++)
                {
                    xone[j] = xone_add_n[j];
                }
                xone[SIZE + 1] =  xone_add_n[SIZE + 2]; // copy sign
            }
            ms_divide_2(xone,xone,SIZE+1);
            fbu = u[0];
        }
        ...
        while ((fbv&1) == 0)
        {
            ...
            if ((fbx&1) == 1)
            {
                ...
                uint32_t xtwo_add_n[SIZE + 3];
                signed_add(xtwo, N, xtwo_add_n, SIZE +1);
                for (uint8_t j =0; j < SIZE+1; j++)
                {
                    xtwo[j] = xtwo_add_n[j];
                }
                xtwo[SIZE + 1] =  xtwo_add_n[SIZE + 2];
            }
            ms_divide_2(xtwo,xtwo,SIZE+1);
            fbv = v[0];
        }
    ...
    signed_mod_n(inverse, res, SIZE + 1);
}
